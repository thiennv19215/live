    ...
    <section className="queue-panel">
      <div className="queue-title">
        <div>
          ...
          <small>{status.current ? `1 đang phát · ${pendingCount} chờ` : `${pendingCount} chờ`}</small>
        </div>
        <button className="icon-button" onClick={() => post("/api/queue/clear")} title="Dừng action và xóa hàng đợi"><Trash2 size={15} /></button>
      </div>
      <div className="queue-list">
        {items.length ? items.map((item, index) => (
          <div className={`queue-row ${index === 0 && status.current ? "active" : ""}`} key={`${item.gift}-${item.file}-${index}`}>
            <span className="queue-index">{String(index + 1).padStart(2, "0")}</span>
            <div><strong>{item.gift}{index === 0 && status.current ? " · Đang phát" : ""}</strong><span>{item.file}</span></div>
            <b>P{item.priority}</b>
          </div>
